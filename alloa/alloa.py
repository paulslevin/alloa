...
edges1 = config['working_files'] + 'edges1_' + date[0] + date[1] + date[3] + \
         date[4] + date[6] + date[7] + '.py'
...
edges2 = config['working_files'] + 'edges2_' + date[0] + date[1] + date[3] + \
         date[4] + date[6] + date[7] + '.py'
...
edges3 = config['working_files'] + 'edges3_' + date[0] + date[1] + date[3] + \
         date[4] + date[6] + date[7] + '.py'
...
if hierarchy_weight == 0:
    allocation_results = [[inv_students[u], inv_projects[v - n * 2],
                           inv_supervisors[s - n * 2 - num_of_proj * 2], 'N/A',
                           'N/A'] for u in J if u > 0 if u < n + 1 for m in J[u]
                          if m > n if m < n * 2 + 1 for v in J[m] if v > n * 2
                          if v < n * 2 + num_of_proj + 1 if J[m][v] > 0 for l in
                          J[v] if l > n * 2 + num_of_proj if
                          l < n * 2 + num_of_proj * 2 + 1 if J[v][l] > 0 for s
                          in J[l] if s > n * 2 + num_of_proj * 2 if
                          s < n * 2 + num_of_proj * 2 + num_of_sup + 1 if
                          J[l][s] > 0]
elif hierarchy_weight == 1:
    allocation_results = [[inv_students[u], inv_projects[v - n * 2],
                           inv_supervisors[s - n * 2 - num_of_proj * 2],
                           int(round(math.log(G[m][v]['weight'], n), 0) + 1),
                           'N/A'] for u in J if u > 0 if u < n + 1 for m in J[u]
                          if m > n if m < n * 2 + 1 for v in J[m] if v > n * 2
                          if v < n * 2 + num_of_proj + 1 if J[m][v] > 0 for l in
                          J[v] if l > n * 2 + num_of_proj if
    ...
    allocation_results = [[inv_students[u], inv_projects[v - num_lines * 2],
                ...
                round(math.log(G[l][s]['weight'], num_lines), 0) + 1)] for u in
                          ...
                          if v > num_lines * 2 if
                          v < num_lines * 2 + num_of_proj + 1 if J[m][v] > 0 for
                          l in J[v] if l > num_lines * 2 + num_of_proj if
                          l < num_lines * 2 + num_of_proj * 2 + 1 if J[v][l] > 0
                          for s in J[l] if s > num_lines * 2 + num_of_proj * 2
                          if
                          s < num_lines * 2 + num_of_proj * 2 + num_of_sup + 1
                          if J[l][s] > 0]
...
allocation_results2 = [
    [inv_supervisors[u - num_lines * 2 - num_of_proj * 2 - num_of_sup], J[u][v]]
    for u in J for v in J[u] if J[u][v] >= 0 if
    u > num_lines * 2 + num_of_proj * 2 + num_of_sup if
    u < num_of_sup * 2 + num_lines * 2 + num_of_proj * 2 + 1]
allocation_results3 = [[inv_projects[u - num_lines * 2], J[u][v]] for u in J for
                       v in J[u] if J[u][v] >= 0 if u > num_lines * 2 if
                       u < num_of_proj + num_lines * 2 + 1]
